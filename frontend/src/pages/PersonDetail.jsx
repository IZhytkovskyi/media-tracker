...
import { 
  ...
  Globe, Database, Tv, ChevronLeft, ChevronRight, Star 
} from 'lucide-react';
...
// Словник жанрів TMDB для української мови
const TMDB_GENRES = {
  28: "Бойовик",
  12: "Пригоди",
  16: "Мультфільм",
  ...
  10752: "Військовий",
  37: "Вестерн",
  10759: "Бойовик та Пригоди",
  10762: "Дитячий",
  10763: "Новини",
  10764: "Реаліті-шоу",
  10765: "Sci-Fi та Фентезі",
  10766: "Мильна опера",
  10767: "Ток-шоу",
  10768: "Війна та Політика"
};
...
export default function PersonDetail() {
  ...
  const unifiedCredits = useMemo(() => {
    ...
    (personData.combined_credits.cast || []).forEach(item => {
      const translatedGenres = (item.genre_ids || []).map(id => TMDB_GENRES[id]).filter(Boolean);
      ...
      projectMap.get(item.id).castRoles.push(item.character || 'Без імені');
    });
  ...
  const sourceGroups = [
    { title: 'Офіційні', icon: <Globe size={20} color="#38bdf8" />, items: [{ label: 'Особистий сайт', url: personData.homepage, icon: <LinkIcon size={18} /> }] },
        ...
        <div style={styles.leftColumn}>
          ...
          <div style={{ ...styles.detailsBox, padding: '15px' }}>
            ...
            <div style={{ display: 'flex', gap: '15px', flexWrap: 'wrap' }}>
              {personData.homepage && (
                <a href={personData.homepage} target="_blank" rel="noopener noreferrer" style={{ color: '#38bdf8' }} title="Особистий сайт">
                  <LinkIcon size={24} />
                </a>
              )}
              {links.imdb_id && (
        ...
        <div style={styles.rightColumn}>
          ...
          {activeTab === 'main' && (
            <>
              ...
              {/* Повна фільмографія з новим універсальним фільтром */}
              <div style={{...styles.sectionBlock, marginTop: '40px'}}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '15px', marginBottom: '20px' }}>
                  <h3 style={{...styles.sectionTitle, marginBottom: 0}}>Повна фільмографія</h3>
                  
                  <AdvancedFilter items={unifiedCredits} onFilterChange={setFilteredCredits} />
                </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  ...
                  {filteredCredits.length === 0 && (
                    <p style={styles.emptyText}>За обраними фільтрами нічого не знайдено.</p>
                  )}
                </div>
              </div>
            </>

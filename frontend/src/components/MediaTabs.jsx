...
// ВИПРАВЛЕНО: правильний імпорт з react-router-dom
import { useNavigate } from 'react-router-dom'; 
import { styles } from '../styles/mediaDetailStyles';

// --- Допоміжні функції форматування та перекладу ---
...
// Допоміжна функція для локальної дати (виправлення проблеми з часовим поясом)
    ...
    <>
      ...
      <div style={styles.genresContainer}>
        {media.genres && media.genres.length > 0 ? (
          ...
          <span style={styles.genreBadge}>Без жанру</span>
        )}
      </div>
      ...
      <div style={styles.detailsBox}>
        ...
        <InfoRow label="Прем'єра в світі:">
        ...
        <div style={styles.sectionBlock}>
          ...
          <div className="custom-scroll" style={styles.horizontalScroll}>
            {tmdbData.seasons.map((season) => (
              ...
              >
                <img src={season.poster_path || 'https://via.placeholder.com/105x155?text=Немає+постера'} alt={season.name} style={styles.personPhoto} />
          ...
          <div className="custom-scroll" style={styles.horizontalScroll}>
            {tmdbData.cast.slice(0, 12).map(actor => (
              <div key={`actor-min-${actor.id}`} style={styles.personCard}>
                <img src={actor.profile_path || 'https://via.placeholder.com/105x155?text=Фото'} alt={actor.name} style={styles.personPhoto} />
...
export function TabActors({ tmdbData }) {
  ...
  const crewSubTabs = useMemo(() => {
    ...
    const jobGroups = {
      'Режисура': ['Director', 'Creator'], 'Сценарій': ['Screenplay', 'Writer', 'Story'],
      'Оператори': ['Director of Photography', 'Camera Operator'], 'Композитори': ['Original Music Composer', 'Music'],
      'Продюсери': ['Producer', 'Executive Producer', 'Co-Producer'], 'Художники': ['Production Design', 'Art Direction', 'Set Decoration'],
      'Монтаж': ['Editor'], 'Костюми та Макіяж': ['Costume Design', 'Makeup Artist', 'Hairstylist'],
    ...
    tmdbData.crew.forEach(member => {
      ...
      for (const [groupName, jobList] of Object.entries(jobGroups)) {
        if (jobList.includes(member.original_job)) {
          existingGroups.add(groupName); isGrouped = true; break;
        }
      }
    ...
    <>
      <div className="sub-tabs-container">
        {crewSubTabs.map(tab => {
          ...
          return (
            <button key={tab.id} className={`sub-tab ${activeCrewTab === tab.id ? 'active' : ''}`} onClick={() => setActiveCrewTab(tab.id)}>{tab.label}</button>
          );
        })}
      </div>
          ...
          <div style={styles.peopleGrid}>
            {paginatedCrew.map((person, idx) => (
              <div key={`${activeCrewTab}-${person.id}-${idx}`} style={styles.gridPersonCard}>
                <img src={person.profile_path || 'https://via.placeholder.com/105x155?text=Фото'} alt={person.name} style={styles.personPhoto} />
...
export function TabShots({ tmdbData }) {
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      if (e.key === 'ArrowRight') setLightboxIndex(prev => Math.min(prev + 1, displayedImages.length - 1));
      ...
      {lightboxIndex !== null && (
        <div style={{
          ...
          <button 
            ...
            style={{ position: 'absolute', left: '20px', background: 'rgba(255,255,255,0.1)', border: 'none', color: '#fff', padding: '15px', borderRadius: '50%', cursor: lightboxIndex === 0 ? 'default' : 'pointer', opacity: lightboxIndex === 0 ? 0.3 : 1, transition: '0.2s' }}
          ...
          <img 
            ...
            alt="Повний екран"
            style={{ maxWidth: '90vw', maxHeight: '90vh', objectFit: 'contain', borderRadius: '8px', boxShadow: '0 10px 40px rgba(0,0,0,0.8)' }}
          />

          <button 
            onClick={(e) => { e.stopPropagation(); setLightboxIndex(prev => Math.min(prev + 1, displayedImages.length - 1)); }}
            style={{ position: 'absolute', right: '20px', background: 'rgba(255,255,255,0.1)', border: 'none', color: '#fff', padding: '15px', borderRadius: '50%', cursor: lightboxIndex === displayedImages.length - 1 ? 'default' : 'pointer', opacity: lightboxIndex === displayedImages.length - 1 ? 0.3 : 1, transition: '0.2s' }}
            disabled={lightboxIndex === displayedImages.length - 1}
          ><ChevronRight size={32} /></button>
          
          <div style={{ position: 'absolute', bottom: '20px', color: '#94a3b8', fontSize: '14px', letterSpacing: '1px' }}>
...
export function TabPremiere({ tmdbData }) {
  const releaseTypeNames = {
    1: 'Прем\'єра', 2: 'Театральний (обмежений)', 3: 'Театральний',
  ...
  return (
    <div style={styles.premiereContainer}>
      <h3 style={styles.sectionTitle}>Дати виходу у країнах</h3>
            ...
            <div key={countryRelease.country} style={{ 
              ...
              <div style={{ ...styles.countryCardHeader, borderBottomColor: isUkraine ? 'rgba(56, 189, 248, 0.2)' : '#334155' }}>
                ...
                <h4 style={{ ...styles.countryName, fontSize: '16px', fontWeight: isUkraine ? 'bold' : 'normal' }}>
                  ...
                  {isProducer && !isUkraine && <span style={{ fontSize: '11px', color: '#94a3b8', marginLeft: '8px', fontWeight: 'normal' }}>(Виробник)</span>}
                </h4>
              </div>
                  ...
                  <div key={idx} style={{ ...styles.dateItem, padding: '8px 0', borderBottom: idx !== countryRelease.dates.length - 1 ? '1px dashed #1e293b' : 'none' }}>
                    <div style={styles.dateTypeWrapper}>
                      <span style={{ ...styles.dateTypeBadge, backgroundColor: dateItem.type === 4 ? '#8b5cf6' : (dateItem.type === 3 ? '#2ecc71' : '#475569'), color: '#fff' }}>
                        {releaseTypeNames[dateItem.type] || 'Інше'}
                      </span>
...
export function TabSources({ tmdbData }) {
  ...
  const sourceGroups = [
    ...
    { title: 'Соціальні мережі', icon: <ExternalLink size={20} color="#2ecc71" />, items: [{ label: 'Instagram', url: links.instagram, icon: <ExternalLink size={18} /> }, { label: 'Twitter (X)', url: links.twitter, icon: <ExternalLink size={18} /> }, { label: 'Facebook', url: links.facebook, icon: <ExternalLink size={18} /> }, { label: 'TikTok', url: links.tiktok, icon: <ExternalLink size={18} /> }] }
  ];
  return (
...
export function TabHistory({ logs, onDelete, onCreate, onUpdate }) {
  ...
  const openEditModal = (log) => { 
    ...
    setFormData({ watch_date: log.watch_date || '', rating: log.rating !== null ? log.rating : '' }); 
    setIsModalOpen(true); 
  };
  
  const handleSubmit = (e) => {
    e.preventDefault();
    const payload = { watch_date: formData.watch_date || null, rating: formData.rating === '' ? null : parseFloat(formData.rating) };
    if (editingLogId) onUpdate(editingLogId, payload); else onCreate(payload);
  ...
  return (
    <div>
      ...
      {(!logs || logs.length === 0) ? (
        <p style={styles.emptyText}>Історія переглядів порожня. Відмітьте фільм як переглянутий або додайте запис вручну.</p>
      ) : (
          ...
          {logs.map((log, index) => (
            <div key={log.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#1a1a1a', padding: '16px 20px', borderRadius: '12px', border: index === 0 ? '1px solid #38bdf8' : '1px solid #333' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                <div style={{ backgroundColor: '#2a2a2a', padding: '10px', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Calendar size={20} color="#a3a3a3" /></div>
                <div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginTop: '6px' }}>
                    <Star size={14} color={log.rating ? "#facc15" : "#555"} fill={log.rating ? "#facc15" : "none"} />
                    <span style={{ color: log.rating ? '#facc15' : '#555', fontSize: '14px', fontWeight: '500' }}>{log.rating ? `${log.rating} з 5` : 'Без оцінки'}</span>
                  </div>
                </div>
              </div>
              <div style={{ display: 'flex', gap: '10px' }}>
                <button onClick={() => openEditModal(log)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '8px', color: '#a3a3a3', transition: 'color 0.2s' }}><Pen size={18} /></button>
                <button onClick={() => onDelete(log.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '8px', color: '#ef4444', transition: 'opacity 0.2s' }}><Trash2 size={18} /></button>
              </div>
            </div>
          ))}
        </div>
          ...
          <div style={{ backgroundColor: '#1e293b', padding: '25px', borderRadius: '12px', width: '100%', maxWidth: '400px', border: '1px solid #334155' }}>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                <label style={{ color: '#94a3b8', fontSize: '13px' }}>Дата перегляду (необов'язково)</label>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                ...
                <input type="number" step="0.5" min="0" max="5" placeholder="Без оцінки" value={formData.rating} onChange={e => setFormData({...formData, rating: e.target.value})} style={{ backgroundColor: '#0f172a', border: '1px solid #334155', color: '#fff', padding: '10px', borderRadius: '8px', outline: 'none' }} />
              </div>
